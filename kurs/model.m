...
%Передача номера 274251, ответ "абонент свободен"
...
sig=sig_o+sig_p+wgn(1,length(sig_p),-40);%сигнал в линии (прямой+обратный+шум)


figure(1)
plot(1:length(sig_p),sig_p,'-r',1:length(sig_o),sig_o,'-b');
title('сигналы прямого и обратного направления');
h = legend('Прямое направление','Обратное направление',2);
